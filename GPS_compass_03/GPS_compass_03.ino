...
unsigned long GPSInterval = 1000; // check GPS at this interval
...
int targetHeading = 0;
int headingsDiff = 0;
int targetRange = 5; // margin of error for within range of target
...
void loop(void)
{
  ...
  if (millis() - previousGPSTime > GPSInterval) { //
    ...
    targetHeading = (int)gps.course_to(flat, flon, Place_LAT, Place_LON);  //calculate the direction we should be walking in
  ...
  //else {  //do all the other feedback stuff
    ...
    if (headingsDiff <= targetRange) {  //if your heading is close enough to the target heading, then make something happen
